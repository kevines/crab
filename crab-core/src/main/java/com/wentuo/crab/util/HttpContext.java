...
//        }
    ...
    public static AppUser getUser() {
        ...
        AppUser user = null;
        try {
            object = RedisUtil.getHashKey(RedisKeys.user.getKey(Constant.TOKEN), getToken());
            if (object != null) {
                user = JSONObject.parseObject((String) object, AppUser.class);
            }
        } catch (Exception e) {
     ...
     * @Date 2018/7/23 下午3:44
     */
    public static String getParamUserId() {
        Object object = RedisUtil.getHashKey(RedisKeys.user.getKey(Constant.TOKEN), getParamToken());
        if (object != null) {
            AppUser user = JSONObject.parseObject((String) object, AppUser.class);
    ...
    public static String getParamToken() {
        ServletRequestAttributes requestAttributes = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
        final String authToken = requestAttributes.getRequest().getParameter("token");
        return authToken;
    }
    ...
    public static Boolean isTokenExpired() {
        ...
        try {
            AppUser appUser = getUser();
            if (appUser.getTokenExpireTime() != null) {
                int sum = DateUtils.checkDatetime(appUser.getTokenExpireTime());
                String hashKey = RedisKeys.user.getKey(Constant.TOKEN);
                if (sum == 1) {
                    RedisUtil.delete(hashKey, getToken());
                    return true;
                }
            }
        } catch (Exception e) {
            logger.info("token过期" + getToken());
        }
        return false;
    }
